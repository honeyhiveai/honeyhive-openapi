...
The official [OpenAPI](https://www.openapis.org/) specification for the [HoneyHive API](https://api.dp1.us.honeyhive.ai).
...
| [`CHANGELOG.md`](CHANGELOG.md) | Markdown | Per-version changelog, deterministically generated by [oasdiff](https://github.com/oasdiff/oasdiff) (`oasdiff changelog --format markdown`) |
...
`CHANGELOG.md` entries are generated deterministically with [oasdiff](https://github.com/oasdiff/oasdiff) by diffing the previously published spec against the new one (`oasdiff changelog --format markdown`), so re-running the same command on the same inputs always produces byte-identical output.
